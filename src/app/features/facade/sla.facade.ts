...
import { SlaApiService } from '@features/service/sla.api.service';
import { SlaModel, SlaUpsertInput } from '@models/sla.models';

@Injectable({ providedIn: 'root' })
export class SlaFacade {
  private readonly api = inject(SlaApiService);

  private readonly _loading = signal(false);
  private readonly _loadedOnce = signal(false);
  private readonly _items = signal<SlaModel[]>([]);
  ...
  create(input: SlaUpsertInput): Observable<SlaModel> {
    return this.api.create(input).pipe(tap(() => this.load()));
  }

  update(id: string, input: SlaUpsertInput): Observable<SlaModel> {
