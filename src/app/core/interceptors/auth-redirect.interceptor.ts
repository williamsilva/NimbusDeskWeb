...
export function isNimbusDesk(url: string): boolean {
...
export function isAutoReloginCandidate(
  ...
  if (!isNimbusDesk(req.url)) return false;
...
function canTriggerLoginNow(): boolean {
  const key = 'nd_login_redirect_lock';
  const now = Date.now();
...
export const authRedirectInterceptor: HttpInterceptorFn = (
  ...
  if (!isNimbusDesk(req.url)) return next(req);
